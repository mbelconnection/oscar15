...
DROP TABLE casemgmt_cpp CASCADE CONSTRAINTS;
CREATE TABLE casemgmt_cpp (
  id NUMBER(10) ,
  CONSTRAINT casemgmt_cpp PRIMARY KEY (id),
  demographic_no varchar(10)  default '',
  socialHistory CLOB,
  familyHistory CLOB,
  medicalHistory CLOB,
  ongoingConcerns CLOB,
  reminders CLOB,
  update_date date default SYSDATE,
  primaryPhysician varchar(255),
  primaryCounsellor varchar(255),
  otherFileNumber varchar(100) ,
  otherSupportSystems CLOB ,
  pastMedications CLOB
);
...
DROP TABLE CASEMGMT_ISSUE_NOTES CASCADE CONSTRAINTS;
CREATE TABLE CASEMGMT_ISSUE_NOTES (
  ID NUMBER(10),
  CONSTRAINT casemgmt_issue_notes PRIMARY KEY (id, note_id),
  NOTE_ID NUMBER(10)
);
CREATE INDEX CASEMGMT_ISSUE_NOTES_NOTE_ID ON CASEMGMT_ISSUE_NOTES(NOTE_ID);

DROP TABLE CASEMGMT_NOTE CASCADE CONSTRAINTS;
CREATE TABLE CASEMGMT_NOTE (
  NOTE_ID NUMBER(10) ,
  CONSTRAINT casemgmt_note PRIMARY KEY (note_id),
  UPDATE_DATE DATE,
  DEMOGRAPHIC_NO VARCHAR(20) ,
  PROVIDER_NO VARCHAR(20) ,
  NOTE CLOB ,
  SIGNED NUMBER(1)  DEFAULT 0,
  INCLUDE_ISSUE_INNOTE NUMBER(1) DEFAULT 0,
  SIGNING_PROVIDER_NO VARCHAR(20),
  ENCOUNTER_TYPE VARCHAR(100),
  BILLING_CODE VARCHAR(100),
  PROGRAM_NO VARCHAR(20),
  AGENCY_NO VARCHAR(20),
  REPORTER_CAISI_ROLE VARCHAR(20),
  REPORTER_PROGRAM_TEAM VARCHAR(20),
  HISTORY CLOB ,
  PASSWORD VARCHAR(255) ,
  LOCKED CHAR(1) ,
  UUID CHAR(36)
);
CREATE INDEX CASEMGMT_NOTE_PROVIDER_NO ON CASEMGMT_NOTE(PROVIDER_NO);
