...
public class PatientDocumentDao extends AbstractDao<PatientDocument> {
	...
	/**
	 * Finds all Patient Document objects for a specific patient (without pagination)
	 * 
	...
	/**
	 * Checks to see if a documentUniqueId for a repositoryUniqueId exists or not.
	...
	public int getDocumentCount(int demographicId) {
		return findPatientDocuments(demographicId).size();
	}
	
	/**
	 * Finds a list of Patient Document objects for a patient with pagination (limit and offset)
	 * 
	 * @param demographicId Oscar's patient ID
