...
public class ConsultationDao extends AbstractDao<ConsultationRequest> {
	...
	private StringBuilder generateQuery(PaginationQuery paginationQuery, boolean selectCountOnly) {
		...
		StringBuilder sql = new StringBuilder("select "+ (selectCountOnly?"count(*)":"cr") +" from ConsultationRequest cr left outer join cr.professionalSpecialist specialist, ConsultationServices cs, Demographic d left outer join d.provider p where d.DemographicNo = cr.demographicId and cs.id = cr.serviceId ");
		if (StringUtils.isNotBlank(consultationQuery.getProviderNo())) {
			sql.append("and cr.providerNo = '" + consultationQuery.getProviderNo() + "' ");
		}
		...
		if (StringUtils.isNotBlank(consultationQuery.getKeyword())) {
			sql.append("and (");
			sql.append("d.LastName like '%" + consultationQuery.getKeyword() + "%' ");
			sql.append("or d.FirstName like '%" + consultationQuery.getKeyword() + "%' ");
			sql.append("or specialist.lastName like '%" + consultationQuery.getKeyword() + "%' ");
			sql.append("or specialist.firstName like '%" + consultationQuery.getKeyword() + "%' ");
			sql.append("or cs.serviceDesc like '%" + consultationQuery.getKeyword() + "%' ");
			sql.append(") ");
		}
