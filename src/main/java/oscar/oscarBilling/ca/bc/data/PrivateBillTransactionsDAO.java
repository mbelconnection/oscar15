...
public class PrivateBillTransactionsDAO extends AbstractDao<BillingPrivateTransactions> {
	...
	public List<PrivateBillTransaction> getPrivateBillTransactions(String billingmaster_no) {
		Query query = entityManager.createNativeQuery("select bp.id, billingmaster_no, amount_received, creation_date, payment_type_id, bt.payment_type  from billing_private_transactions bp,billing_payment_type bt where bp.billingmaster_no = :billingmasterNo and bp.payment_type_id = bt.id");
