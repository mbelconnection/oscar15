...
<br>
<i><bean:message key="demographic.demographicsearchresults.msgSearchKeys" /></i> : <%=searchText%>
    ...
    <table>
		...
		<!-- leave blank -->
                    ...
                    <a href="demographiccontrol.jsp?fromMessenger=<%=fromMessenger%>&keyword=<%=searchText%>&displaymode=<%=request.getParameter("displaymode")%>&search_mode=<%=request.getParameter("search_mode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=demographic_no&limit1=0&limit2=<%=strLimit%>"><bean:message
		...
		<td class="links"><bean:message key="demographic.demographicsearchresults.module" /></td>

		<%}%>
		<td class="name"><a
                    href="demographiccontrol.jsp?fromMessenger=<%=fromMessenger%>&keyword=<%=searchText%>&displaymode=<%=request.getParameter("displaymode")%>&search_mode=<%=request.getParameter("search_mode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=last_name&limit1=0&limit2=<%=strLimit%>"><bean:message
                    key="demographic.demographicsearchresults.btnDemoName"/></a>
                </td>
		<td class="chartNo"><a
			href="demographiccontrol.jsp?fromMessenger=<%=fromMessenger%>&keyword=<%=searchText%>&displaymode=<%=request.getParameter("displaymode")%>&search_mode=<%=request.getParameter("search_mode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=chart_no&limit1=0&limit2=<%=strLimit%>"><bean:message
			key="demographic.demographicsearchresults.btnChart" /></a>
                </td>
		<td class="sex"><a
			href="demographiccontrol.jsp?fromMessenger=<%=fromMessenger%>&keyword=<%=searchText%>&displaymode=<%=request.getParameter("displaymode")%>&search_mode=<%=request.getParameter("search_mode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=sex&limit1=0&limit2=<%=strLimit%>"><bean:message
			key="demographic.demographicsearchresults.btnSex" /></a>
                </td>
		<td class="dob"><a
			href="demographiccontrol.jsp?fromMessenger=<%=fromMessenger%>&keyword=<%=searchText%>&displaymode=<%=request.getParameter("displaymode")%>&search_mode=<%=request.getParameter("search_mode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=dob&limit1=0&limit2=<%=strLimit%>"><bean:message
		...
		<td class="doctor"><a
			href="demographiccontrol.jsp?fromMessenger=<%=fromMessenger%>&keyword=<%=searchText%>&displaymode=<%=request.getParameter("displaymode")%>&search_mode=<%=request.getParameter("search_mode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=provider_no&limit1=0&limit2=<%=strLimit%>"><bean:message
			key="demographic.demographicsearchresults.btnDoctor" /></a>
                </td>
                <td class="rosterStatus"><a
			href="demographiccontrol.jsp?fromMessenger=<%=fromMessenger%>&keyword=<%=searchText%>&displaymode=<%=request.getParameter("displaymode")%>&search_mode=<%=request.getParameter("search_mode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=roster_status&limit1=0&limit2=<%=strLimit%>"><bean:message
                        key="demographic.demographicsearchresults.btnRosSta" /></a>
                </td>
		<td class="patientStatus"><a
			href="demographiccontrol.jsp?fromMessenger=<%=fromMessenger%>&keyword=<%=searchText%>&displaymode=<%=request.getParameter("displaymode")%>&search_mode=<%=request.getParameter("search_mode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=patient_status&limit1=0&limit2=<%=strLimit%>"><bean:message
                        key="demographic.demographicsearchresults.btnPatSta" /></a>
                </td>
                <td class="phone"><a
			href="demographiccontrol.jsp?fromMessenger=<%=fromMessenger%>&keyword=<%=searchText%>&displaymode=<%=request.getParameter("displaymode")%>&search_mode=<%=request.getParameter("search_mode")%>&dboperation=<%=request.getParameter("dboperation")%>&orderby=phone&limit1=0&limit2=<%=strLimit%>"><bean:message
	...
	demoList = doSearch(demographicDao,searchMode,ptstatus,searchText,limit,offset,orderBy,providerNo,outOfDomain);	
...
<%
  ...
  if(nLastPage>=0) {
%> 
	<a href="demographiccontrol.jsp?keyword=<%=searchText%>&search_mode=<%=searchMode%>&displaymode=<%=displayMode%>&dboperation=<%=dboperation%>&orderby=<%=orderBy%>&limit1=<%=nLastPage%>&limit2=<%=strLimit%>&ptstatus=<%=ptStatus%>&firstPageShowIntegratedResults=<%=firstPageShowIntegratedResults%><%=nLastPage==0 && firstPageShowIntegratedResults?"&includeIntegratedResults=true":""%>">
	<bean:message key="demographic.demographicsearchresults.btnLastPage" /></a> <%
  }
  if(nItems>=Integer.parseInt(strLimit)) {
      if (nLastPage>=0) {
	%> | <%    } %> 
	<a href="demographiccontrol.jsp?keyword=<%=searchText%>&search_mode=<%=searchMode%>&displaymode=<%=displayMode%>&dboperation=<%=dboperation%>&orderby=<%=orderBy%>&limit1=<%=nNextPage%>&limit2=<%=strLimit%>&ptstatus=<%=ptStatus%>&firstPageShowIntegratedResults=<%=firstPageShowIntegratedResults%>">
...
<br> 
<div class="createNew">
<a href="demographicaddarecordhtm.jsp?search_mode=<%=searchMode%>&keyword=<%=searchText%>" title="<bean:message key="demographic.search.btnCreateNewTitle" />">
