...
g_jsDatePickImagePath = "js_up/img/";
...
JsDatePick.prototype.setConfiguration = function (a) {
	...
	this.oConfiguration.disablePreDays = (a.disablePreDays != null) ? a.disablePreDays : false;
...
JsDatePick.prototype.populateMainBox = function (h) {
    ...
    while (j.getMonth() == n) {
		...
		if (this.oConfiguration.disablePreDays) { /*Added by Bhaskar */
            if (this.isAvailable(this.currentYear, this.currentMonth, parseInt(j.getDate()+1))) {
                k = true;
                g.setAttribute("isJsDatePickDisabled", 1)
            }
        }
...
JsDatePick.prototype.getSelectedDayFormatted = function () {
    if (this.flag_aDayWasSelected) {
        ...
        a = a.replace("%M", g_l.MONTHS[this.selectedDayObject.month - 1].substr(0, 3));
