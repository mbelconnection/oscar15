...
dtmlXMLLoaderObject.prototype.getXMLTopNode = function (a, b) {
    if (this.xmlDoc.responseXML) {
        ...
        var d = c[0]
    } else d = this.xmlDoc.documentElement; if (d) return this._retry = !1, d;
        ...
        var c = [],
            d = function () {
                for (var d = !0, f = 0; f < c.length; f++)
                    if (c[f] != null) var g = c[f].apply(a, arguments),
                d = d && g;
                return d
            };
    ...
    markRow: function (a, b, c) {
        ...
        if (this.callEvent("onRowMark", [a, b, c, e]) && (d = this.styles[b ? c : "clear"] + d, this.obj[this._methods[0]](a, d), e && e.details)) {
            ...
            for (var f = 0; f < e.details.length; f++)
                if (e.details[f]) this.obj[this._methods[1]](a, f, d)
        }
    },
    getState: function (a) {
        ...
        if (this.updatedRows.length) {
            ...
            if (this._tSend) this._sendData(this._getAllData());
                ...
                for (b = 0; b < this.updatedRows.length; b++)
                    if (!this._in_progress[this.updatedRows[b]] && !this.is_invalid(this.updatedRows[b]) && (this._beforeSendData(this._getRowData(this.updatedRows[b]), this.updatedRows[b]), this._waitMode && (this.obj.mytype == "tree" || this.obj._h2))) break
        }
    },
...
scheduler.set_actions = function () {
    for (var a in this._els)
        if (this._click[a])
            for (var b = 0; b < this._els[a].length; b++) this._els[a][b].onclick = scheduler._click[a];
...
scheduler._click = {
    dhx_cal_data: function (a) {
        ...
        } else scheduler.callEvent("onEmptyClick", [scheduler.getActionData(a).date, a]); if (c && scheduler.config.select) {
            scheduler.select(c);
            var d = b.className;
            if (d.indexOf("_icon") != -1) scheduler._click.buttons[d.split(" ")[1].replace("icon_",
                "")](c)
...
scheduler._mouse_coords = function (a) {
    ...
    if (this._table_view) {
        ...
        if (this._drag_mode == "move" && scheduler._ignores_detected &&
            scheduler.config.preserve_length && (b._ignores = !0, !this._drag_event._event_length)) this._drag_event._event_length = this._get_real_event_length(this._drag_event.start_date, this._drag_event.end_date, {
            x_step: 1,
            x_unit: "day"
        });
        b.x = 0
...
scheduler._process_ignores = function (a, b, c, d, e) {
    ...
    if (f)
        for (var g = new Date(a), h = 0; h < b; h++) f(g) && (this._ignores_detected += 1, this._ignores[h] = !0, e && b++), g = scheduler.date.add(g, d, c)
};
...
scheduler.render_data = function (a, b) {
    for (var a = this._pre_render_events(a, b), c = 0; c < a.length; c++)
        if (this._table_view)
            if (scheduler._mode != "month") this.render_event_bar(a[c]);
            else {
    ...
    if (this._table_view)
        if (b) this._colsS.heights = d;
            ...
            if (g.rows) {
                ...
                e[0] = 0;
                if (q != p) f.style.top = parseInt(f.style.top) + 2 + "px", s.style.overflowY = "auto", s.style.width = parseInt(s.style.width) - 2 + "px", u.style.position = "fixed", u.style.top =
                    "", u.style.left = ""
            }
        }
    return a
        ...
        if (this._ignores[f._sday]) a.splice(e, 1), e--;
                    ...
                    if (j.length)
                        if (j.length <= j[j.length - 1]._sorder) {
                            ...
                            for (o = 1; o < j.length; o++)
                                if (j[o]._sorder > p) p = j[o]._sorder;
                            f._sorder = p + 1;
                            f._inner = !1
                        } else f._sorder = 0;
                j.push(f);
            ...
            if (k < this.config.first_hour || i >= this.config.last_hour)
                if (d.push(f), a[e] = f = this._copy_event(f), k < this.config.first_hour && (f.start_date.setHours(this.config.first_hour), f.start_date.setMinutes(0)), i >= this.config.last_hour &&
                    (f.end_date.setMinutes(0), f.end_date.setHours(this.config.last_hour)), f.start_date > f.end_date || k == this.config.last_hour) a.splice(e, 1), e--
        }
    }
    if (!b) {
...
scheduler.render_event = function (a) {
    ...
    if (!(a._sday < 0)) {
        ...
        if (d) {
            ...
            m = this.config.cascade_event_margin, l = h % n * m, o = a._inner ? (g - h - 1) % n * m / 2 : 0, j = Math.floor(d.clientWidth - c - l - o);
            ...
            if (this._edit_id ==
                ...
                if (this._quirks7) q.firstChild.style.height = i -
                    12 + "px";
            ...
            if (this.xy.menu_width !== 0 && this._select_id == a.id) {
                ...
                for (var r =
                    this.config["icons_" + (this._edit_id == a.id ? "edit" : "select")], s = "", u = a.color ? "background-color: " + a.color + ";" : "", t = a.textColor ? "color: " + a.textColor + ";" : "", v = 0; v < r.length; v++) s += "<div class='dhx_menu_icon " + r[v] + "' style='" + u + "" + t + "' title='" + this.locale.labels[r[v]] + "'></div>";
...
scheduler._render_v_bar = function (a, b, c, d, e, f, g, h, k) {
    ...
    if (k || !scheduler.renderEvent(q, j)) {
        ...
        console.log(m+">>"+o+">>"+g);
        r += '<div class="dhx_event_move dhx_title" style="' + m + "" + o + 'height:20px;">' + g + "</div>";
        r += '<div class="dhx_body" style=" width:' + (d - (this._quirks ? 4 : 4)) + "px; height:" + (e - (this._quirks ? 20 : 20) + 1) + "px;" + m + "" + o + '">' + h + "</div>";
        var s = "dhx_event_resize dhx_footer";
        k && (s = "dhx_resize_denied " + s);
        r += '<div class="' + s + '" style=" width:' + (d - 8) + "px;" + (k ? "" : "") + "" +
            m + "" + o + '" ></div>';
        q.innerHTML = r
    }
    return q
...
scheduler._load = function (a, b) {
    if (a = a || this._load_url) {
        ...
        if (this._load_mode) {
            ...
            c = b;
            for (var e = !0; c < this._max_date;) c = this.date.add(c, 1, this._load_mode), this._loaded[d(b)] && e ? b = this.date.add(b, 1, this._load_mode) :
                e = !1;
            var f = c;
            do c = f, f = this.date.add(c, -1, this._load_mode); while (f > b && this._loaded[d(f)]);
            if (c <= b) return !1;
...
scheduler.json.parse = function (a) {
    ...
    var b = scheduler._temp && scheduler._temp.collections ? scheduler._temp.collections : {}, c = !1,
        d;
    for (d in b)
        if (b.hasOwnProperty(d)) {
            ...
            if (f) {
                ...
                for (var g = 0; g < e.length; g++) {
                    ...
                    for (i in h)
                        if (h.hasOwnProperty(i) && !(i == "value" || i == "label")) k[i] = h[i];
                    f.push(k)
                }
            }
        }
