...
public class EaapsHandlerTest extends DaoTestFixtures {
	...
	public void testProviderParsing() throws Exception {
		...
		if (demo == null) {
			...
			demo.setYearOfBirth("2000");
			
			demoDao.save(demo);
		}
		
		StudyDao studyDao = SpringUtils.getBean(StudyDao.class);
		Study study = findStudy("eAAPS Study");
		if (study == null) {
			study = new Study();
			study.setCurrent1(1);
		...
		MessagetblDAO mtDao = (MessagetblDAO) SpringUtils.getBean("MessagetblDAO");
		List<Messagetbl> messages = mtDao.getHibernateTemplate().find("from Messagetbl mtbl");
	...
	private Study findStudy(String name) {
		StudyDao studyDao = SpringUtils.getBean(StudyDao.class);
		for(Study s : studyDao.findAll()) {
			if (name.equals(s.getStudyName())) {
				return s;
			}
		}
	    return null;
