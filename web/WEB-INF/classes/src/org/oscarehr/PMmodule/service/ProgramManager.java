...
public class ProgramManager {
    ...
    public Agency getAgencyByProgram(String programId) {
        return new Agency((long) 0, 1, "HS", "HS", "", true, false);
    }
    ...
    public List<ProgramProvider> getProgramProvidersByProvider(String providerNo) {
        List<ProgramProvider> pp = programProviderDAO.getProgramProvidersByProvider(Long.valueOf(providerNo));
        for (ProgramProvider aPp : pp) {
            String name = getProgramName(String.valueOf(aPp.getProgramId()));
            if (name == null) {
                log.warn("Program doesn't have a name?");
                name = "";
            }
            aPp.setProgramName(name);
        }
