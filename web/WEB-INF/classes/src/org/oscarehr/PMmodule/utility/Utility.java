...
//###################################################################################
  ...
  public static String escapeURL(String str)
  {
    ...
    str=replaceURL(str,"\\","/");  // for mainpic updates in experiences table
  	return str;
  }
...
//###################################################################################
public static String convertToRelacementStrIfNull(String str, String replaceWith)
{
    if(str != null)
    {
        return str;
...
public static String[] separateStrComponentsIntoStrArray(String str, String delimiter)
{

// strTokens[10] = ~MINP4~MAXA4~MAXP6~MAXC2~INFY~ROOMTYPEPHS~OCCUPCODEQAD
    String strComponents = str;

    int count = 0;
    StringTokenizer strTokenizer = new StringTokenizer(strComponents, delimiter);
...
public static String[] getParamsFromPosting(HttpServletRequest request, String keyPrefix) 
{
// this method only filters out the keyNames that has a nonempty keyValue -- others don't work
	...
	try
	{
        Enumeration params = request.getParameterNames(); //receives all the posted params
		...
		{
			      //Display an error message saying that at least 1 checkbox should be checked!
        	return null;
		}
...
public static String[][] getKeyNamesAndKeyValuesFromPosting(HttpServletRequest request, String keyPrefix) 
{
// this method only filters out the keyNames that has a nonempty keyValue -- others don't work
...
public static String[] getKeyNameSuffixFromPosting(HttpServletRequest request, String keyPrefix) 
{
// this method only filters out the keyNames that has a nonempty keyValue -- others don't work
	...
	try
	{
        ...
        while(params.hasMoreElements())
	    {
            ...
            if(keyName.startsWith(keyPrefix))
            {
            	...
            	{
            		keyNameSuffix = keyName.substring( keyPrefix.length() , keyName.length());
            		keyValue = request.getParameter(keyName);
            	}
			    if(!keyValue.equals("")  &&  keyNameSuffix.length() > 0) //ie. checked checkboxes
				{
...
public static List filterOutDuplicateStrTokens(List duplStrList)
{
    ...
    for(int i=0; i < duplStrList.size(); i++)
    {
        ...
        {
            if( compareToken.length() > 0  &&  compareToken.equalsIgnoreCase( (String)duplStrList.get(j) ) )
            {
            	duplStrList.set(j, "");
            }
        }
...
{
	log.debug("Utility/filterOutDuplicateStrTokens():  filteredStrList == null");

}
else
{
log.debug("Utility/filterOutDuplicateStrTokens():  filteredStrList.size() = " + filteredStrList.size());
}
