...
public class CaseManagementViewAction extends BaseCaseManagementViewAction
{
	...
	public ActionForward view(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		/*
		 * EncounterWindow ectWin = this.caseManagementMgr.getEctWin(providerNo); if (ectWin == null) { ectWin = new EncounterWindow();
		 * ectWin.setProvider_no(providerNo); ectWin.setRowOneSize(EncounterWindow.NORMAL); ectWin.setRowTwoSize(EncounterWindow.NORMAL); }
		 * 
		...
		if (tab.equals("Current Issues"))
		{
			 ...
			 * request.setAttribute("Issues",caseManagementMgr.filterIssues(issues,providerNo,programId)); else request.setAttribute("Issues",issues);
			 */
			...
			// copy cpp notes
			/*
			 * HashMap issueMap = getCPPIssues(request, providerNo); Iterator<Map.Entry> iterator = issueMap.entrySet().iterator(); while( iterator.hasNext() )
			 * { Map.Entry mapEntry = iterator.next(); String key = (String)mapEntry.getKey(); Issue value = (Issue)mapEntry.getValue();
			 * List<CaseManagementNote>cppNotes = caseManagementMgr.getCPP(demoNo,value.getId(),userProp); String cppAdd = request.getContextPath() +
			 * "/CaseManagementEntry.do?hc=996633&method=issueNoteSave&providerNo=" + providerNo + "&demographicNo=" + demoNo + "&issue_id=" + value.getId() +
			 * "&noteId="; request.setAttribute(key,cppNotes); request.setAttribute(key+"add",cppAdd); }
			 */
			...
			/*
			 * people are changing the default sorting of notes so it's safer to explicity set it here, some one already changed it once and it reversed our
			 * sorting.
			 */
