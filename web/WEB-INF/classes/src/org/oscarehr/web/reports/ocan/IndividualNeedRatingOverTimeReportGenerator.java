...
public class IndividualNeedRatingOverTimeReportGenerator {
	...
	public void generateReport(OutputStream os) throws Exception  {
		...
		if(reportBean.isShowUnmetNeeds()) {
			d.add(Image.getInstance(reportBean.getUnmetNeedsChart().createBufferedImage((int)PageSize.A4.rotate().width()-40, height), null));
		}

		if(reportBean.isShowMetNeeds()) {
			d.add(Image.getInstance(reportBean.getMetNeedsChart().createBufferedImage((int)PageSize.A4.rotate().width()-40, height), null));
		}

		if(reportBean.isShowNoNeeds()) {
			d.add(Image.getInstance(reportBean.getNoNeedsChart().createBufferedImage((int)PageSize.A4.rotate().width()-40, height), null));
		}
		if(reportBean.isShowUnknownNeeds()) {
			d.add(Image.getInstance(reportBean.getUnknownNeedsChart().createBufferedImage((int)PageSize.A4.rotate().width()-40, height), null));
		}
	...
	private JFreeChart generateChart(String needType) {
		return null;
	}
