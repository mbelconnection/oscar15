...
public class FrmONARRecord extends FrmRecord {
    public Properties getFormRecord(int demographicNo, int existingID) throws SQLException {
        ...
        try {
            ...
            if (existingID <= 0) {
                ...
                String sql = "SELECT demographic_no, last_name, first_name, sex, " + "address, city, province, postal, phone, phone2, " + "year_of_birth, month_of_birth, date_of_birth, hin, chart_no FROM demographic WHERE demographic_no = " + demographicNo;
                ResultSet rs = db.searchDBRecord(c, sql);
                if (rs.next()) {
                    ...
                    props.setProperty("pg1_age", String.valueOf(UtilDateUtilities.calcAge(rs.getString("year_of_birth"), rs.getString("month_of_birth"), rs.getString("date_of_birth"))));
    ...
    public boolean isSendToPing(String demoNo) throws SQLException {
        ...
        if ("yes".equalsIgnoreCase(OscarProperties.getInstance().getProperty("PHR", ""))) {
            ...
            if (rs.next()) {
                if (rs.getString("email") != null && rs.getString("email").length() > 5 && rs.getString("email").matches(".*@.*")) ret = true;
            }
