...
public final class LoginAction
    ...
    private static final String FORWARD_PROVIDER = "provider";
    private static final String FORWARD_ADMIN = "admin";
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form,
        ...
        if (!cl.propFileFound) {
            String newURL = mapping.findForward(FORWARD_ERROR).getPath();
        ...
        if (strAuth != null && strAuth.length != 1) { //login successfully
            ...
            if (viewType.equalsIgnoreCase("receptionist")) { // go to receptionist view
                ...
                where = FORWARD_PROVIDER;
            }
            else if (viewType.equalsIgnoreCase("doctor")) { // go to provider view
                where = FORWARD_PROVIDER; //providercontrol.jsp?year="+nowYear+"&month="+(nowMonth)+"&day="+(nowDay)+"&view=0&displaymode=day&dboperation=searchappointmentday";
            }
            else if (viewType.equalsIgnoreCase(FORWARD_ADMIN)) { // go to admin view
                where = FORWARD_ADMIN;
            }
        ...
        if (OscarProperties.getInstance().isTorontoRFQ() && where.equals(FORWARD_PROVIDER)){
            where = "caisiProvider";
        }
