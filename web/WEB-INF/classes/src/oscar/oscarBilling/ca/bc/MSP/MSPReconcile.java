...
public class MSPReconcile {
    ...
    public String getS00String(String billingMasterNo) {
        ...
        try {
            ...
            String sql = "SELECT teleplanS00.t_exp1,teleplanS00.t_exp1,teleplanS00.t_exp2,teleplanS00.t_exp3,teleplanS00.t_exp4,teleplanS00.t_exp5,teleplanS00.t_exp6,teleplanS00.t_exp7 FROM teleplanS00 " + "where t_officeno = '" + forwardZero(billingMasterNo, 7) + "'";
            ResultSet rs = db.GetSQL(sql);
            while (rs.next()) {
                String exp[] = new String[7];
                exp[0] = rs.getString("t_exp1");
    ...
    public Map getS00Map() {
        ...
        try {
            DBHandler db = new DBHandler(DBHandler.OSCAR_DATA);
            ResultSet rs = db.GetSQL("SELECT t_dataseq,CAST(t_officeno as SIGNED INTEGER) FROM teleplans00");
            while (rs.next()) {
    ...
    public BillSearch getBills(String statusType, String providerNo, String startDate, String endDate, String demoNo, boolean excludeWCB, boolean excludeMSP, boolean excludePrivate, boolean exludeICBC) {
        ...
        if ("?".equals(statusType) || "$".equals(statusType)) {
            if ("?".equals(statusType)) {
                statusTypeClause += " in ('" + this.PAIDWITHEXP + "','" + this.REJECTED + "','" + this.HELD + "')";
            }
            else if ("$".equals(statusType)) {
                statusTypeClause += " in ('" + this.PAIDWITHEXP + "','" + this.PAIDPRIVATE + "','" + this.SETTLED + "')";
            }
        }
        else {
        ...
        //
        String p = " select b.billing_no, b.demographic_no, b.demographic_name, b.update_date, b.billingtype," + " b.status, b.apptProvider_no,b.appointment_no, b.billing_date,b.billing_time, bm.billingstatus, " + " bm.bill_amount, bm.billing_code, bm.dx_code1, bm.dx_code2, bm.dx_code3," + " b.provider_no, b.visitdate, b.visittype,bm.billingmaster_no,p.first_name,p.last_name,bm.billing_unit from billing b left join provider p on p.provider_no = b.provider_no, "
                + " billingmaster bm where b.billing_no= bm.billing_no "

                + statusTypeClause + providerQuery + startDateQuery + endDateQuery + demoQuery + billingType + " order by b.billing_date desc";
    ...
    private double getTotalPaidFromS00(String billingmaster_no) {
        double retval = 0.0;
        String qry = "select  t_paidamt from teleplanS00 where t_officeno =  '" + forwardZero(billingmaster_no, 7) + "'";
    ...
    private double getTotalPaidFromHistory(String billingmaster_no, boolean ignoreIA) {
        ...
        double retval = 0.0;
        String historyQry = "select  sum(amount_received) from billing_history where billingmaster_no = " + billingmaster_no;
        if (ignoreIA) {
    ...
    public void saveOrUpdateBillRecipient(BillRecipient recip) throws SQLException {
        ...
        try {
            DBHandler db = new DBHandler(DBHandler.OSCAR_DATA);
            rs = db.GetSQL(c, "select count(*) from bill_recipients where billingNo = " + recip.getBillingNo());

            //Record exists so perform an update
            if (rs.next()) {
                ps = c.prepareStatement("update bill_recipients set name=?,address=?,city=?,province=?,postal=?,updateTime=now() where billingNo=?");
                ps.setString(1, recip.getName());
                ps.setString(2, recip.getAddress());
                ps.setString(3, recip.getCity());
                ps.setString(4, recip.getProvince());
                ps.setString(5, recip.getPostal());
                ps.setString(6, recip.getBillingNo());
            }
            else {
                //create a new record
                ps = c.prepareStatement("insert into bill_recipients(name,address,city,province,postal,creationTime,updateTime,billingNo) " + "values(?,?,?,?,?,now(),now(),?)");
                ps.setString(1, recip.getName());
                ps.setString(2, recip.getAddress());
                ps.setString(3, recip.getCity());
                ps.setString(4, recip.getProvince());
    ...
    private void updateBillingStatusHlp2(String billingNo, String stat) {
        try {
            DBHandler db = new DBHandler(DBHandler.OSCAR_DATA);
            db.RunSQL("update billingmaster set billingstatus = '" + stat + "' where billingmaster_no = '" + billingNo + "'");
            db.CloseConn();
        }
        catch (Exception e) {
            e.printStackTrace();
    ...
    private void updatePrivateBillState() {
        ...
        try {
            ...
            db.RunSQL("UPDATE billing b SET b.billingtype = 'Pri' where b.billingtype = 'PRIV'");
            String findPrivs = "select b.billing_no " + "from billing b,billingmaster bm " + "where b.billingtype = 'Pri' " + "and b.billing_no = bm.billing_no " + "and bm.billingstatus not in('" + this.BILLPATIENT + "','" + this.PAIDPRIVATE + "')";
        ...
        if (privateTransactions != null) {
            ...
            try {
                ...
                for (Iterator iter = privateTransactions.iterator(); iter.hasNext();) {
                    String[] item = (String[])iter.next();
                    String recExists = "select * from billing_history where " + "billingmaster_no = " + item[0] + " and " + "amount_received = " + item[1] + " and " + "creation_date = '" + item[2] + "' and " + "payment_type_id = " + item[3];
                    ResultSet rs = db.GetSQL(recExists);
                    //if the audit entry doesn't exist, create it.
                    if (!rs.next()) {
                        String insert = "insert into billing_history(billingmaster_no,amount_received,creation_date,payment_type_id) " + " values(" + item[0] + "," + item[1] + ",'" + item[2] + "'," + item[3] + ")";
                        db.RunSQL(insert);
                    }
                }
            }
    ...
    private void updateBillTypeHlp(String billingNo, String billType) {
        String updateBillingSQL = "update billing set billingtype = '" + billType + "' where billing_no ='" + billingNo + "'";
        DBHandler db = null;
        try {
            db = new DBHandler(DBHandler.OSCAR_DATA);
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (type.equals(REP_REJ)) {
                    ...
                    ResultSet rsDemo = db.GetSQL("select phone,phone2 from demographic where demographic_no = " + b.demoNo);
                    if (rsDemo.next()) {
    ...
    public double getAmountOwing(String billingMasterNo, String amountBilled, String billingType) {
        ...
        if (!this.BILLTYPE_PRI.equalsIgnoreCase(billingType)) {
            ...
            String qry = "SELECT t_paidamt,t_exp1 from teleplanS00" + " where teleplanS00.t_officeno =  '" + this.forwardZero(billingMasterNo, 7) + "'";
            try {
                db = new DBHandler(DBHandler.OSCAR_DATA);
                rs = db.GetSQL(qry);
                while (rs.next()) {
    ...
    private String getC12Description(String code) {
        String desc = "";
        String qry = "select description from teleplan_refusal_code where code = '" + code + "'";
        DBHandler db = null;
        ResultSet rs = null;
        try {
            db = new DBHandler(DBHandler.OSCAR_DATA);
    ...
    public MSPReconcile.BillSearch getPayments(String account, String payeeNo, String providerNo, String startDate, String endDate, boolean excludeWCB, boolean excludeMSP, boolean excludePrivate, boolean exludeICBC) {
        BillSearch billSearch = new BillSearch();
        String criteriaQry = createCriteriaString(account, payeeNo, providerNo, UtilMisc.replace(startDate, "-", ""), UtilMisc.replace(endDate, "-", ""), excludeWCB, excludeMSP, excludePrivate, exludeICBC, this.REP_PAYREF, "");
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (!this.BILLTYPE_PRI.equalsIgnoreCase(b.billingtype)) {
                    ...
                    b.amount = String.valueOf(getAmountPaid(rs.getString("bm.billingmaster_no"), this.BILLTYPE_PRI));
                ...
                if (!"".equals(b.adjustmentCode)) {
                    ...
                    if (b.adjustmentCodeDesc.equals("")) {
                        System.out.println("no description for b.adjustmentCode:" + b.adjustmentCode);
                    }
                }
    ...
    private String getPaymentMethodDesc(String id) {
        ...
        DBHandler db = null;
        ResultSet rs = null;
        String qry = "select payment_type from billing_payment_type where id = " + id;
        try {
            db = new DBHandler(DBHandler.OSCAR_DATA);
            rs = db.GetSQL(qry);
    ...
    private String createCriteriaString(String account, String payeeNo, String providerNo, String startDate, String endDate, boolean excludeWCB, boolean excludeMSP, boolean excludePrivate, boolean exludeICBC, String repType, String dateFieldOption) {
        String criteriaQry = "";
        String dateField = this.REP_PAYREF.equals(repType)?"t_payment":"service_date";
    ...
    public ResultSet getMSPRemittanceQuery(String payeeNo, String s21Id) {
        System.out.println(new java.util.Date() + ":MSPReconcile.getMSPRemittanceQuery(payeeNo, s21Id)");
    ...
    public oscar.entities.Provider getProvider(String providerNo, int criteria) {
        ...
        String qry = "select first_name,last_name from provider where " + criteriaStr + " = " + providerNo;
        try {
            db = new DBHandler(DBHandler.OSCAR_DATA);
            rs = db.GetSQL(qry);
    ...
    public List getAllProviders() {
        ...
        String qry = "select * from provider where provider_type = 'doctor'";
        try {
            db = new DBHandler(DBHandler.OSCAR_DATA);
            System.err.println(qry);
            rs = db.GetSQL(qry);
    ...
    public S21 getS21Record(String s21id) {
        System.out.println(new java.util.Date() + ":MSPReconcile.getS21Record(s21id)");
        String qry = "select t_payment,t_payeeno,t_payeename,t_amtbilled,t_amtpaid,t_cheque from teleplanS21 where status <> 'D' and s21_id = " + s21id + " order by t_payment desc";
        DBHandler db = null;
        ResultSet rs = null;
        S21 s21 = new S21();
        try {
    ...
    public boolean patientHasOutstandingPrivateBill(String demographicNo) {
        boolean ret = false;
        String billingMasterQry = "select billingmaster_no,bill_amount from billingmaster bm,billing b where bm.billing_no = b.billing_no  and b.demographic_no = " + demographicNo + " and bm.billingstatus not in('S','D','A') and b.billingtype = '" + this.BILLTYPE_PRI + "'";
        DBHandler db = null;
        ResultSet rs = null;
        try {
            db = new DBHandler(DBHandler.OSCAR_DATA);
    ...
    public void settleIfBalanced(String billingmasterNo) {
        String[] row = SqlUtils.getRow("SELECT b1.billingtype, b.bill_amount " + "FROM billingmaster b, billing b1 " + "WHERE b1.billing_no=b.billing_no " + "AND billingmaster_no = " + billingmasterNo);
        if (row != null && row.length > 0) {
    ...
    public void settleBGBills() {
        ...
        String bgqry = "SELECT t_officeno,t_billamt FROM teleplanS00 t " + " WHERE t.t_exp1 = 'BG' " + " or t.t_exp2 = 'BG' " + " or t.t_exp3 = 'BG' " + " or t.t_exp4 = 'BG' " + " or t.t_exp5 = 'BG' " + " or t.t_exp6 = 'BG' " + " or t.t_exp7 = 'BG' ";
