...
function ts1(s) {
popupPage2(('../appointment/addappointment.jsp?'+s),'appointment',360,780);
}
function tsr(s) {
popupPage2(('../appointment/appointmentcontrol.jsp?displaymode=edit&dboperation=search&'+s),'appointment',360,780);
}
...
<security:oscarSec roleName="<%=roleName$%>" objectName="_appointment.doctorLink" rights="r">
 ...
 <caisi:isModuleLoad moduleName="TORONTO_RFQ" reverse="true">
     <li>
	 <a HREF="#" ONCLICK ="popupOscarRx(600,1090,'../oscarMessenger/DisplayMessages.do?providerNo=<%=curUser_no%>&userName=<%=URLEncoder.encode(userfirstname+" "+userlastname)%>')" title="<bean:message key="global.messenger"/>">
	 <span id="oscar_new_msg"><bean:message key="global.msg"/></span></a>
     </li>
 </caisi:isModuleLoad>
<caisi:isModuleLoad moduleName="TORONTO_RFQ" reverse="true">
<li>
 <a HREF="#" ONCLICK ="popupOscarRx(625,1090,'../oscarEncounter/IncomingConsultation.do?providerNo=<%=curUser_no%>&userName=<%=URLEncoder.encode(userfirstname+" "+userlastname)%>')" title="<bean:message key="provider.appointmentProviderAdminDay.viewConReq"/>">
...
<oscar:oscarPropertiesCheck property="WORKFLOW" value="yes">
 ...
 <oscar:oscarPropertiesCheck property="MY_OSCAR" value="yes">
    ...
    <li>
	<a HREF="#" ONCLICK ="popup('600', '1090','../phr/PhrMessage.do?method=viewMessages','INDIVOMESSENGER2<%=curUser_no%>')" title='<bean:message key="global.myoscar"/>'><phr:setColor><bean:message key="global.btnmyoscar"/></phr:setColor></a>
    </li>
...
<td BGCOLOR="ivory" width="33%">
 <a href="providercontrol.jsp?year=<%=year%>&month=<%=month%>&day=<%=isWeekView?(day-7):(day-1)%>&view=<%=view==0?"0":("1&curProvider="+request.getParameter("curProvider")+"&curProviderName="+request.getParameter("curProviderName") )%>&displaymode=day&dboperation=searchappointmentday<%=isWeekView?"&provider_no="+provNum:""%>&viewall=<%=viewall%>">
...
<%--|--%>
        ...
        %>
    		...
    		<%} %>
<a href=# onClick ="popupPage2('../appointment/appointmentcontrol.jsp?appointment_no=<%=appointment.get("appointment_no")%>&provider_no=<%=curProvider_no[nProvider]%>&year=<%=year%>&month=<%=month%>&day=<%=day%>&start_time=<%=iS+":"+iSm%>&demographic_no=0&displaymode=edit&dboperation=search','appointment',360,780,860);return false;" title="<%=iS+":"+(iSm>10?"":"0")+iSm%>-<%=iE+":"+iEm%>
<%=name%>
...
<%String warning = prevMgr.getWarnings(String.valueOf(demographic_no));
 ...
 %>
<%=htmlWarning%><a href=# onClick ="popupPage2('../appointment/appointmentcontrol.jsp?appointment_no=<%=appointment.get("appointment_no")%>&provider_no=<%=curProvider_no[nProvider]%>&year=<%=year%>&month=<%=month%>&day=<%=day%>&start_time=<%=iS+":"+iSm%>&demographic_no=<%=demographic_no%>&displaymode=edit&dboperation=search','appointment',360,780,860);return false;"  <oscar:oscarPropertiesCheck property="SHOW_APPT_REASON_TOOLTIP" value="yes" defaultVal="true"> title="<%=name%>
...
<!-- caisi infirmary view extension modify end ffffffffffffffffff-->
 ...
 %>
      ...
      <table BORDER="0" CELLPADDING="0" CELLSPACING="0" WIDTH="100%">
           ...
           <a href="providercontrol.jsp?year=<%=year%>&month=<%=month%>&day=<%=isWeekView?(day+7):(day+1)%>&view=<%=view==0?"0":("1&curProvider="+request.getParameter("curProvider")+"&curProviderName="+request.getParameter("curProviderName") )%>&displaymode=day&dboperation=searchappointmentday<%=isWeekView?"&provider_no="+provNum:""%>">
           <img src="../images/next.gif" WIDTH="10" HEIGHT="9" BORDER="0" ALT="<bean:message key="provider.appointmentProviderAdminDay.viewNextDay"/>" vspace="2">&nbsp;&nbsp;</a>
        <a href=# onClick ="popupPage(425,430,'../share/CalendarPopup.jsp?urlfrom=../provider/providercontrol.jsp&year=<%=strYear%>&month=<%=strMonth%>&param=<%=URLEncoder.encode("&view=0&displaymode=day&dboperation=searchappointmentday","UTF-8")%><%=isWeekView?URLEncoder.encode("&provider_no="+provNum, "UTF-8"):""%>')"><bean:message key="global.calendar"/></a></td>
...
document.onkeydown=function(e){
	...
	if (evt.altKey) {
		...
		switch(evt.keyCode) {
			...
			case <bean:message key="global.labShortcut"/> : popupOscarRx(600,1350,'../dms/inboxManage.do?method=prepareForIndexPage&providerNo=<%=curUser_no%>', '<bean:message key="global.lab"/>');  return false;  //run code for 'L'ab
			case <bean:message key="global.msgShortcut"/> : popupOscarRx(600,1090,'../oscarMessenger/DisplayMessages.do?providerNo=<%=curUser_no%>&userName=<%=URLEncoder.encode(userfirstname+" "+userlastname)%>'); return false;  //run code for 'M'essage
			...
			case <bean:message key="global.conShortcut"/> : popupOscarRx(625,1090,'../oscarEncounter/IncomingConsultation.do?providerNo=<%=curUser_no%>&userName=<%=URLEncoder.encode(userfirstname+" "+userlastname)%>');  return false;  //run code for c'O'nsultation
			...
			case <bean:message key="global.myoscarShortcut"/> : popupOscarRx('600', '1090','../phr/PhrMessage.do?method=viewMessages','INDIVOMESSENGER2<%=curUser_no%>')
			default : return;
               }
	}
	if (evt.ctrlKey) {
